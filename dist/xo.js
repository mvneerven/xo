(function (global, factory) {
  ...
  _defineProperty(ExoForm, "meta", {
    ...
    templates: {
      ...
      default:
      /*html*/
      `<div data-id="{{id}}" class="exf-ctl-cnt {{class}}"><label title="{{caption}}">{{caption}}</label><span data-replace="true"></span></div>`,
      nolabel:
      /*html*/
      `<div data-id="{{id}}" class="exf-ctl-cnt {{class}}"><span data-replace="true"></span></div>`,
      text:
      /*html*/
      `<div data-id="{{id}}" class="exf-ctl-cnt exf-base-text {{class}}"><span data-replace="true"></span><label title="{{caption}}">{{caption}}</label></div>`,
      labelcontained:
      /*html*/
      `<div data-id="{{id}}" class="exf-ctl-cnt {{class}}"><label title="{{tooltip}}"><span data-replace="true"></span> <span>{{caption}}</span></label></div>`,
      form:
      /*html*/
  ...
  class ExoInputListControl extends ExoListControl {
    ...
    async render() {
      ...
      const tpl =
      /*html*/
      `<label title="{{tooltip}}"><input class="{{class}}" {{checked}} name="{{inputname}}" value="{{value}}" type="{{type}}" /><span class="exf-caption">{{name}}</span></label>`;
      await this.populateList(this.htmlElement, tpl);
      super.render();
      return this.container;
    }
  ...
  class ExoSwitchControl extends ExoBaseControls.controls.range.type {
    ...
    setProperties() {
      ...
      this.context.field.max = 1; //this.context.field.containerClass = "exf-switch";
...
<div class="exf-dlg-f">
  ...
  class ExoFormFactory {
    ...
    static listNativeProps(ctl) {
      ...
      let ar = ["style", "class", "accesskey", "contenteditable", "dir", "disabled", "hidefocus", "lang", "language", "tabindex", "title", "unselectable", " xml:lang"];
      list.forEach(p => {
